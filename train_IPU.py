...
def get_train_loader(trainset, world_size, batch_size):
    if world_size <= 1:
        #return torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=CFG.DATA.NUM_WORKERS, pin_memory=True)  # Original GPU
        return poptorch.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=CFG.DATA.NUM_WORKERS, pin_memory=True)
    elif world_size > 1:
        ...
        ##Create a poptorch DataLoader object with the Distributed distribution mode. poptorch.DataLoader automatically creates a distributed sampler
        poptorch_loader = poptorch.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=CFG.DATA.NUM_WORKERS, pin_memory=True, options=poptorch.DataLoaderOptions().with_distribution(poptorch.DistributionMode.Distributed))
